import { clientPost, setConfig } from "./client.js";
import { getToolsForProvider, executeToolInternal } from "./tools.js";
export function configure(options) {
    if (options.url !== undefined)
        setConfig({ url: options.url });
}
export function memkit(model) {
    const provider = model.startsWith("gpt-") || model.startsWith("o1-") ? "openai" : "anthropic";
...
async function normalizeAddInput(items) {
    ...
    if (Array.isArray(items) && items.length > 0) {
        ...
        if (typeof first === "string") {
            const documents = await Promise.all(items.map((s) => resolveDocument(s)));
            return { documents };
        }
...
async function resolveDocument(s) {
